<template>
  ...
  <AppLayout
    v-if="!isModalInspection"
    :title="
      inspectionType === 'post-trip'
        ? store.t('postTripInspection')
        : store.t('preTripInspection')
    "
  >
    <RouterLink
      to="/driver"
    ...
    <div class="card p-4 mb-4">
      ...
      <div class="relative mb-4">
        ...
        />
        <span
          class="absolute right-3 top-1/2 -translate-y-1/2 text-xs text-gray-500 dark:text-gray-400 uppercase"
          >{{ odometerUnit }}</span
        >
      </div>
      ...
      <div
        v-if="odometerWarning && !odometerConfirmed"
        class="mt-3 rounded-lg border border-yellow-300 bg-yellow-50 dark:border-yellow-700 dark:bg-yellow-900/20 p-3"
      >
        ...
        <button
          type="button"
          class="mt-2 text-xs font-semibold text-yellow-700 dark:text-yellow-400 underline underline-offset-2"
          @click="odometerConfirmed = true"
        >
          Yes, the value is correct — confirm
        </button>
      </div>

      <!-- Engine hours input (only if required by template) -->
      <div v-if="engineHoursRequired" class="mt-6">
        <label class="block text-xs mb-1 font-semibold"
          >Engine hours (for heavy/industrial vehicles)</label
        >
        <input
          v-model="engineHoursInput"
          type="number"
          min="0"
          step="1"
    ...
    <div class="card p-4 mb-4">
      <div
        class="flex justify-between text-xs text-gray-500 dark:text-gray-400 mb-1.5"
      >
        <span
          >{{ doneCount }} / {{ items.length }} {{ store.t("checked") }}</span
        >
      ...
      <div class="flex items-center justify-between mt-2">
        <div class="flex gap-4 text-xs">
          <span class="text-green-600 dark:text-green-400"
            >{{ passCount }} {{ store.t("okLabel") }}</span
          >
          <span class="text-red-500 dark:text-red-400"
            >{{ failCount }} {{ store.t("issuesLabel") }}</span
          >
        </div>
        <button
          @click="markAllPass"
    ...
    <div class="card divide-y divide-gray-100 dark:divide-gray-700 mb-4">
      <div
        ...
        class="p-4"
        :class="
          validationErrors[item.id] ? 'bg-red-50/60 dark:bg-red-900/10' : ''
        "
      >
        <div class="flex items-center justify-between gap-3">
          ...
          >
            ...
            <div class="min-w-0">
              <div class="flex flex-wrap items-center gap-2">
                ...
                <span v-if="item.requiresPhoto" class="badge-orange"
                  >Photo</span
                >
              </div>
              <p
          ...
          <div class="flex gap-2 flex-shrink-0">
            ...
            <button
              @click.stop="setState(item, 'not_applicable')"
        ...
        <div
          ...
          class="mt-2 ml-7 flex flex-wrap items-center gap-3"
        >
          <label
            class="flex items-center gap-1.5 text-xs font-medium text-orange-600 dark:text-orange-400 hover:text-orange-700 dark:hover:text-orange-300 transition-colors cursor-pointer"
          >
        ...
        <Transition name="slide">
          <div v-if="isExpanded(item.id)" class="mt-3 ml-7 space-y-3">
            ...
            <div
              v-if="item.referencePhotoUrl"
              class="rounded-lg border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900/40 p-2"
            >
              <p
                class="text-xs font-medium text-gray-500 dark:text-gray-400 mb-2"
              >
                {{ store.t("referencePhoto") }}
              </p>
              <button
                type="button"
                class="h-24 w-24 overflow-hidden rounded-lg border border-gray-200 dark:border-gray-700"
                @click="openPhotoLightbox([item.referencePhotoUrl], 0)"
              >
                <img
            ...
            <textarea
              ...
              :class="
                item.state === 'fail'
                  ? 'placeholder-red-300 dark:placeholder-red-700 border-red-200 dark:border-red-800 bg-red-50 dark:bg-red-900/10'
                  : ''
              "
            />
            <div
            ...
            >
              <label
                class="flex items-center gap-1.5 text-xs text-gray-500 dark:text-gray-400 hover:text-blue-600 dark:hover:text-blue-400 transition-colors cursor-pointer"
              >
                <Camera :size="13" /> {{ store.t("addPhoto") }}
                <input
                  type="file"
    ...
    <div class="card p-4 mb-4">
      <div class="mb-3 flex items-start justify-between gap-3">
        <div>
          ...
          <p class="text-xs text-gray-500 dark:text-gray-400">
            Sign by hand on the screen to submit this inspection.
          </p>
        </div>
        <button
          type="button"
      ...
      <div
        class="rounded-xl border border-gray-200 bg-white p-2 dark:border-gray-700 dark:bg-gray-900/40"
      >
        <canvas
          ref="signatureCanvas"
    ...
    <div class="flex gap-3 pb-4">
      ...
      <button
        ...
        :disabled="submittingInspection || savingDraft"
      >
        {{
          submittingInspection
            ? store.t("submittingInspection")
  ...
  <Teleport v-else to="body">
    ...
    >
      <div class="flex min-h-full justify-center p-4 py-8">
        <div
          class="w-full max-w-xl rounded-2xl bg-white shadow-2xl dark:bg-gray-900 p-4"
        >
          <button
            type="button"
            class="flex items-center gap-2 text-gray-500 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white mb-4 text-sm font-medium transition-colors"
          ...
          <div class="card p-4 mb-4">
            ...
            <p class="mt-2 text-xs text-gray-500 dark:text-gray-400">
              Whole numbers only. The value cannot be less than the last
              odometer.
            </p>
            <p
              v-if="odometerError"
            ...
            >
              <p
                class="text-xs font-medium text-yellow-800 dark:text-yellow-300"
              >
                ⚠️ {{ odometerWarning }}
              </p>
              <button
                ...
                >
                  ...
                  <div class="min-w-0">
                    <div class="flex flex-wrap items-center gap-2">
                      ...
                      <span v-if="item.isRequired" class="badge-blue"
                        >Required</span
                      >
                      <span v-if="item.requiresPhoto" class="badge-orange"
                        >Photo</span
          ...
          <p
            v-if="submitError"
            class="mb-3 text-sm text-red-500 dark:text-red-400"
          >
            {{ submitError }}
          </p>
          <div class="flex gap-3 pb-4">
...
import {
  readSignatureFallback,
  readSignatureFallbackFromDb,
} from "@/lib/signatureFallback";
...
const isModalInspection = computed(
  () => String(route.query.modal || "") === "1",
);
...
const inspectionType = computed<"pre-trip" | "post-trip">(() =>
  inspection.value?.type === "post-trip" || props.isPostTrip
    ? "post-trip"
    : "pre-trip",
);
...
async function loadInspectionItems() {
  ...
  if (!inspectionError) {
    ...
    const vehicleId = String(
      route.query.vehicleId || inspectionData?.vehicle_id || "",
    );
    ...
    const persistedInspectionOdometer =
      inspectionData?.odometer != null &&
      Number.isFinite(Number(inspectionData.odometer))
    ...
    const fallbackSignature =
      ...
      (await readSignatureFallbackFromDb(inspectionId));
    signatureDataUrl.value =
      inspectionData?.signature_data_url || fallbackSignature?.dataUrl || "";
    odometerInput.value =
      inspectionData?.status &&
      inspectionData.status !== "draft" &&
      persistedInspectionOdometer != null
    ...
    const vehicleOdometer =
      vehicle?.odometer != null ? Number(vehicle.odometer) : null;
  ...
  const mappedItems = data.map((result: any) => {
    ...
    return {
      ...
      referencePhotoUrl:
        result.inspection_template_items?.reference_photo_url || null,
      category:
        result.inspection_template_items?.inspection_item_categories?.name ||
        null,
      categorySeverity:
        result.inspection_template_items?.inspection_item_categories
          ?.severity || "medium",
      ...
      icon: FileText,
      state: ["pass", "fail", "not_applicable"].includes(result.result)
        ? result.result
        : null,
  ...
  items.splice(
    0,
    items.length,
    ...mappedItems.sort((a: Item, b: Item) => a.sortOrder - b.sortOrder),
  );
...
const passCount = computed(
  () => items.filter((i) => i.state === "pass").length,
);
const failCount = computed(
  () => items.filter((i) => i.state === "fail").length,
);
const progress = computed(() =>
  items.length ? Math.round((doneCount.value / items.length) * 100) : 0,
);
const vehicleLabel = computed(() => {
  ...
  return (
    [name, vehicle?.unit ? `#${vehicle.unit}` : ""]
      .filter(Boolean)
      .join(" · ") ||
    routeQueryValue("vehicleLabel") ||
    "—"
  );
});
...
const inspectionDateLabel = computed(() =>
  formatDateTime(
    inspection.value?.created_at || routeQueryValue("createdAt"),
    store.language,
  ),
);
...
async function validateOdometerJumpOnSubmit(
  vehicleId: string,
  inspectionId: string,
) {
  const parsed = parseOdometerInput();
  ...
  const floor = await vehicleStore.fetchVehicleOdometerFloor(
    vehicleId,
    inspectionId,
  );
  ...
  if (parsed - floor > 1000) {
    ...
    if (!odometerConfirmed.value) {
      nextTick(() => {
        document
          .querySelector<HTMLElement>(".input-field")
...
async function runPhotoFraudAnalysis(inspectionId: string) {
  ...
  const photos = items.flatMap((item) => {
    const metadata = photoMetaByItemId.value[item.id] || [];
    return item.photos.filter(Boolean).map((dataUrl, photoIndex) => {
...
async function handleSubmit() {
  ...
  try {
    if (inspectionId && vehicleId) {
      ...
      if (!(await validateOdometerJumpOnSubmit(vehicleId, inspectionId)))
        return;
      ...
      try {
        ...
        console.warn(
          "[PreTripInspection] photo fraud analysis failed",
          fraudError,
        );
      }
    }
    ...
    router.push(
      inspectionId
        ? `/inspect/result?inspectionId=${inspectionId}`
        : "/inspect/result",
    );
  } finally {
...
async function handleSaveDraft() {
  ...
  try {
    ...
    if (
      vehicleId &&
      !(await validateOdometerJumpOnSubmit(vehicleId, inspectionId))
    )
      return;
    await saveInspectionResults();
...
async function fetchLatestCommittedOdometer(
  vehicleId?: string,
...
async function fetchMaxHistoricalOdometer(
  vehicleId?: string,
  currentInspectionId?: string,
) {
  if (!vehicleId) return null;
...
async function createIssuesForFailedResults(inspectionId: string) {
  ...
  if (error) {
    console.error(
      "[PreTripInspection] failed to create inspection issues",
      error,
    );
  }
}
